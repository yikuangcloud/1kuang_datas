...
@RestController
@RequestMapping("/chart/BI/pops")
public class ChartPopsBIController {
    ...
    public AjaxResult analysis(@RequestBody ChartDBInfo chartDBInfo) {
        ...
        JSONArray arr = null;
		List<JSONObject> pops = new ArrayList<>();
        try {
            ...
            if(!arr.isEmpty()){
            	...
            	for (int i = 0; i < coordinateList.size(); i++) {
        			...
        			top.put("data", Float.parseFloat(value.toString()) );
        			top.put("name", coordinateList.get(i));
        ...
        return AjaxResult.success(pops);
    }
}
