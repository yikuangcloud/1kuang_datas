...
@RestController
@RequestMapping("/chart/BI/piemore")
public class ChartPieMoreBIController {
        ...
        try {
            ...
            if(!arr.isEmpty()){
	            ...
	            for (int i = 0; i < coordinateList.size(); i++) {
	    			...
	    			pie.put("value", Float.parseFloat(value.toString()) );
	    			pie.put("name", coordinateList.get(i));
	    			List<String> axisData = new ArrayList<>();
	    			for (int j = 0; j < arr.size(); j++) {
	                    JSONObject axisObject = (JSONObject) arr.get(j);
	                    axisData.add((String) axisObject.get("zbwd"));
	                }
	    			pie.put("axisData", axisData);
	    			pies.add(pie);
